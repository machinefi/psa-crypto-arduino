...
#include "include/iotex/platform.h"
		...
		#include "include/iotex/check_crypto_config.h"
	#endif
	...
	#if defined(IOTEX_PSA_BUILTIN_ALG_HKDF) || defined(IOTEX_PSA_BUILTIN_ALG_HKDF_EXTRACT) ||      \
...
iotex_psa_drbg_context_t* const iotex_psa_random_state = &global_data.rng.drbg;
	#endif
...
psa_status_t iotex_to_psa_error(int ret)
{
	...
	#if !(defined(IOTEX_PSA_CRYPTO_EXTERNAL_RNG) || defined(IOTEX_PSA_HMAC_DRBG_MD_TYPE))
...
/****************************************************************/
		...
		defined(IOTEX_PSA_BUILTIN_ALG_DETERMINISTIC_ECDSA) || defined(IOTEX_PSA_BUILTIN_ALG_ECDH)
iotex_ecp_group_id iotex_ecc_group_of_psa(psa_ecc_family_t curve, size_t bits, int bits_is_sloppy)
{
	switch(curve)
	{
...
psa_status_t psa_allocate_buffer_to_slot(psa_key_slot_t* slot, size_t buffer_length)
{
	...
	slot->key.data = (uint8_t*)iotex_calloc(1, buffer_length);
	if(slot->key.data == NULL)
		return (PSA_ERROR_INSUFFICIENT_MEMORY);
									  ...
									  size_t* bits)
{
	...
	else if(PSA_KEY_TYPE_IS_ASYMMETRIC(type))
	{
		...
		{
			return (iotex_psa_ecp_import_key(attributes, data, data_length, key_buffer,
		...
		{
			return (iotex_psa_rsa_import_key(attributes, data, data_length, key_buffer,
											 key_buffer_size, key_buffer_length, bits));
		}
 ...
 */
static psa_status_t psa_get_and_lock_key_slot_with_policy(psa_key_id_t key, psa_key_slot_t** p_slot,
 ...
 */
static psa_status_t psa_get_and_lock_transparent_key_slot_with_policy(psa_key_id_t key,
...
exit:
	...
	#if defined(IOTEX_PSA_BUILTIN_KEY_TYPE_RSA_KEY_PAIR) ||                                        \
		defined(IOTEX_PSA_BUILTIN_KEY_TYPE_RSA_PUBLIC_KEY)
static psa_status_t psa_get_rsa_public_exponent(const iotex_rsa_context* rsa,
...
psa_status_t psa_get_key_attributes(psa_key_id_t key, psa_key_attributes_t* attributes)
{
	...
	attributes->core.flags &= (IOTEX_PSA_KA_MASK_EXTERNAL_ONLY | IOTEX_PSA_KA_MASK_DUAL_USE);
	...
	switch(slot->attr.type)
	{
		...
		case PSA_KEY_TYPE_RSA_PUBLIC_KEY:
			...
			if(!psa_key_lifetime_is_external(slot->attr.lifetime))
			{
				...
				status = iotex_psa_rsa_load_representation(slot->attr.type, slot->key.data,
														   slot->key.bytes, &rsa);
				if(status != PSA_SUCCESS)
					break;
...
psa_status_t psa_export_key(psa_key_id_t key, uint8_t* data, size_t data_size, size_t* data_length)
{
	...
	*data_length = 0;
	...
	status = psa_driver_wrapper_export_key(&attributes, slot->key.data, slot->key.bytes, data,
											...
											uint8_t* data, size_t data_size, size_t* data_length)
{
	...
	#if defined(IOTEX_PSA_BUILTIN_KEY_TYPE_RSA_KEY_PAIR) ||                                        \
		defined(IOTEX_PSA_BUILTIN_KEY_TYPE_RSA_PUBLIC_KEY)
			return (iotex_psa_rsa_export_public_key(attributes, key_buffer, key_buffer_size, data,
													data_size, data_length));
	#else
	...
	#if defined(IOTEX_PSA_BUILTIN_KEY_TYPE_ECC_KEY_PAIR) ||                                        \
		defined(IOTEX_PSA_BUILTIN_KEY_TYPE_ECC_PUBLIC_KEY)
			return (iotex_psa_ecp_export_public_key(attributes, key_buffer, key_buffer_size, data,
													data_size, data_length));
	#else
								   ...
								   size_t* data_length)
{
	...
	*data_length = 0;
	...
	if(data_size < PSA_EXPORT_PUBLIC_KEY_OUTPUT_SIZE(slot->attr.type, slot->attr.bits))
	{
		status = PSA_ERROR_BUFFER_TOO_SMALL;
		goto exit;
	}

	status = psa_driver_wrapper_export_public_key(&attributes, slot->key.data, slot->key.bytes,
...
static psa_status_t psa_validate_key_policy(const psa_key_policy_t* policy)
{
	if((policy->usage &
		~(PSA_KEY_USAGE_EXPORT | PSA_KEY_USAGE_COPY | PSA_KEY_USAGE_ENCRYPT |
		  ...
		  PSA_KEY_USAGE_SIGN_HASH | PSA_KEY_USAGE_VERIFY_HASH | PSA_KEY_USAGE_VERIFY_DERIVATION |
												...
												psa_se_drv_table_entry_t** p_drv)
{
	...
	if(attributes->core.flags & ~(IOTEX_PSA_KA_MASK_EXTERNAL_ONLY | IOTEX_PSA_KA_MASK_DUAL_USE))
	{
		return (PSA_ERROR_INVALID_ARGUMENT);
	}
											...
											psa_key_id_t* key)
{
	...
	if(!PSA_KEY_LIFETIME_IS_VOLATILE(slot->attr.lifetime))
	{
		...
		if(driver != NULL)
		{
			...
			static_assert(sizeof(slot_number) == sizeof(data.slot_number),
													 ...
													 const psa_key_attributes_t* attributes)
{
	...
	if(attributes->domain_parameters_size != 0)
	{
		...
		if(PSA_KEY_TYPE_IS_RSA(slot->attr.type))
		{
			...
			psa_status_t status = iotex_psa_rsa_load_representation(slot->attr.type, slot->key.data,
																	slot->key.bytes, &rsa);
			if(status != PSA_SUCCESS)
				return (status);
			...
			ret = iotex_rsa_export(rsa, NULL, NULL, NULL, NULL, &actual);
			iotex_rsa_free(rsa);
			iotex_free(rsa);
			if(ret != 0)
				goto rsa_exit;
			ret = iotex_mpi_read_binary(&required, attributes->domain_parameters,
...
exit:
	...
	#if defined(IOTEX_PSA_CRYPTO_SE_C)
psa_status_t iotex_psa_register_se_key(const psa_key_attributes_t* attributes)
{
	psa_status_t status;
	psa_key_slot_t* slot = NULL;
	psa_se_drv_table_entry_t* driver = NULL;
...
psa_status_t psa_copy_key(psa_key_id_t source_key, const psa_key_attributes_t* specified_attributes,
						  psa_key_id_t* target_key)
{
	...
	*target_key = IOTEX_SVC_KEY_ID_INIT;
	...
	status = psa_restrict_key_policy(source_slot->attr.type, &actual_attributes.core.policy,
	...
	if(psa_key_lifetime_is_external(actual_attributes.core.lifetime))
	{
		...
		status = psa_driver_wrapper_copy_key(&actual_attributes, source_slot->key.data,
											 source_slot->key.bytes, target_slot->key.data,
											 target_slot->key.bytes, &target_slot->key.bytes);
		if(status != PSA_SUCCESS)
			goto exit;
	}
	else
							 ...
							 size_t input_length)
{
	...
	if(!operation || (!input && input_length != 0))
	{
		status = PSA_ERROR_INVALID_ARGUMENT;
		goto exit;
	}
							 ...
							 size_t hash_length)
{
	...
	psa_status_t status =
		psa_hash_finish(operation, actual_hash, sizeof(actual_hash), &actual_hash_length);
							  ...
							  const uint8_t* hash, size_t hash_length)
{
	...
	psa_status_t status = psa_driver_wrapper_hash_compute(alg, input, input_length, actual_hash,
...
static psa_status_t psa_mac_setup(psa_mac_operation_t* operation, psa_key_id_t key,
								  psa_algorithm_t alg, int is_sign)
{
	...
	if(is_sign)
	{
		status = psa_driver_wrapper_mac_sign_setup(operation, &attributes, slot->key.data,
												   slot->key.bytes, alg);
	}
	else
	{
		status = psa_driver_wrapper_mac_verify_setup(operation, &attributes, slot->key.data,
													 slot->key.bytes, alg);
	}
...
psa_status_t psa_mac_sign_setup(psa_mac_operation_t* operation, psa_key_id_t key,
								psa_algorithm_t alg)
{
	return (psa_mac_setup(operation, key, alg, 1));
}

psa_status_t psa_mac_verify_setup(psa_mac_operation_t* operation, psa_key_id_t key,
								  psa_algorithm_t alg)
{
							...
							size_t input_length)
{
	...
	psa_status_t status = psa_driver_wrapper_mac_update(operation, input, input_length);
	if(status != PSA_SUCCESS)
		psa_mac_abort(operation);
...
static psa_status_t psa_mac_compute_internal(psa_key_id_t key, psa_algorithm_t alg,
											 ...
											 int is_sign)
{
	...
	status =
		psa_driver_wrapper_mac_compute(&attributes, slot->key.data, slot->key.bytes, alg, input,
									   input_length, mac, operation_mac_size, mac_length);
...
{
	return (psa_mac_compute_internal(key, alg, input, input_length, mac, mac_size, mac_length, 1));
}
...
static psa_status_t psa_sign_internal(psa_key_id_t key, int input_is_message, psa_algorithm_t alg,
									  ...
									  size_t signature_size, size_t* signature_length)
{
	...
	*signature_length = 0;
	...
	if(input_is_message)
	{
		status = psa_driver_wrapper_sign_message(&attributes, slot->key.data, slot->key.bytes, alg,
	...
	{
		status =
			psa_driver_wrapper_sign_hash(&attributes, slot->key.data, slot->key.bytes, alg, input,
...
static psa_status_t psa_verify_internal(psa_key_id_t key, int input_is_message, psa_algorithm_t alg,
										const uint8_t* input, size_t input_length,
										const uint8_t* signature, size_t signature_length)
{
	...
	status = psa_get_and_lock_key_slot_with_policy(
		key, &slot, input_is_message ? PSA_KEY_USAGE_VERIFY_MESSAGE : PSA_KEY_USAGE_VERIFY_HASH,
		alg);
	...
	{
		status =
			psa_driver_wrapper_verify_message(&attributes, slot->key.data, slot->key.bytes, alg,
											  input, input_length, signature, signature_length);
	}
	else
	{
		status = psa_driver_wrapper_verify_hash(&attributes, slot->key.data, slot->key.bytes, alg,
												input, input_length, signature, signature_length);
	}
...
psa_status_t psa_verify_message(psa_key_id_t key, psa_algorithm_t alg, const uint8_t* input,
								...
								size_t signature_length)
{
	return psa_verify_internal(key, 1, alg, input, input_length, signature, signature_length);
}
								   ...
								   size_t* signature_length)
{
	if(attributes->core.type == PSA_KEY_TYPE_RSA_KEY_PAIR)
	{
		...
		{
	#if defined(IOTEX_PSA_BUILTIN_ALG_RSA_PKCS1V15_SIGN) || defined(IOTEX_PSA_BUILTIN_ALG_RSA_PSS)
			return (iotex_psa_rsa_sign_hash(attributes, key_buffer, key_buffer_size, alg, hash,
											hash_length, signature, signature_size,
											signature_length));
	...
	else if(PSA_KEY_TYPE_IS_ECC(attributes->core.type))
	{
		...
		{
	#if defined(IOTEX_PSA_BUILTIN_ALG_ECDSA) || defined(IOTEX_PSA_BUILTIN_ALG_DETERMINISTIC_ECDSA)
			return (iotex_psa_ecdsa_sign_hash(attributes, key_buffer, key_buffer_size, alg, hash,
											  hash_length, signature, signature_size,
											  signature_length));
									 ...
									 const uint8_t* signature, size_t signature_length)
{
	if(PSA_KEY_TYPE_IS_RSA(attributes->core.type))
	{
		...
		{
	#if defined(IOTEX_PSA_BUILTIN_ALG_RSA_PKCS1V15_SIGN) || defined(IOTEX_PSA_BUILTIN_ALG_RSA_PSS)
			return (iotex_psa_rsa_verify_hash(attributes, key_buffer, key_buffer_size, alg, hash,
											  hash_length, signature, signature_length));
	#endif /* defined(IOTEX_PSA_BUILTIN_ALG_RSA_PKCS1V15_SIGN) ||                                  \
			* defined(IOTEX_PSA_BUILTIN_ALG_RSA_PSS) */
		}
	...
	else if(PSA_KEY_TYPE_IS_ECC(attributes->core.type))
	{
		...
		{
	#if defined(IOTEX_PSA_BUILTIN_ALG_ECDSA) || defined(IOTEX_PSA_BUILTIN_ALG_DETERMINISTIC_ECDSA)
			return (iotex_psa_ecdsa_verify_hash(attributes, key_buffer, key_buffer_size, alg, hash,
												hash_length, signature, signature_length));
	#endif /* defined(IOTEX_PSA_BUILTIN_ALG_ECDSA) ||                                              \
			* defined(IOTEX_PSA_BUILTIN_ALG_DETERMINISTIC_ECDSA) */
		}
...
{
	return psa_verify_internal(key, 0, alg, hash, hash_length, signature, signature_length);
}

psa_status_t psa_asymmetric_encrypt(psa_key_id_t key, psa_algorithm_t alg, const uint8_t* input,
									...
									uint8_t* output, size_t output_size, size_t* output_length)
{
	...
	*output_length = 0;
	...
	status =
		psa_get_and_lock_transparent_key_slot_with_policy(key, &slot, PSA_KEY_USAGE_ENCRYPT, alg);
	if(status != PSA_SUCCESS)
		return (status);
	...
	status = psa_driver_wrapper_asymmetric_encrypt(&attributes, slot->key.data, slot->key.bytes,
...
psa_status_t psa_asymmetric_decrypt(psa_key_id_t key, psa_algorithm_t alg, const uint8_t* input,
									size_t input_length, const uint8_t* salt, size_t salt_length,
									uint8_t* output, size_t output_size, size_t* output_length)
{
	psa_status_t status = PSA_ERROR_CORRUPTION_DETECTED;
	...
	*output_length = 0;
	...
	status =
		psa_get_and_lock_transparent_key_slot_with_policy(key, &slot, PSA_KEY_USAGE_DECRYPT, alg);
	if(status != PSA_SUCCESS)
		return (status);
	if(!PSA_KEY_TYPE_IS_KEY_PAIR(slot->attr.type))
	{
	...
	status = psa_driver_wrapper_asymmetric_decrypt(&attributes, slot->key.data, slot->key.bytes,
												   alg, input, input_length, salt, salt_length,
												   output, output_size, output_length);
...
static psa_status_t psa_cipher_setup(psa_cipher_operation_t* operation, psa_key_id_t key,
									 psa_algorithm_t alg, iotex_operation_t cipher_operation)
{
	psa_status_t status = PSA_ERROR_CORRUPTION_DETECTED;
	psa_status_t unlock_status = PSA_ERROR_CORRUPTION_DETECTED;
	psa_key_slot_t* slot = NULL;
	psa_key_usage_t usage =
		(cipher_operation == IOTEX_ENCRYPT ? PSA_KEY_USAGE_ENCRYPT : PSA_KEY_USAGE_DECRYPT);
	...
	if(cipher_operation == IOTEX_ENCRYPT)
		status = psa_driver_wrapper_cipher_encrypt_setup(operation, &attributes, slot->key.data,
														 slot->key.bytes, alg);
	else
		status = psa_driver_wrapper_cipher_decrypt_setup(operation, &attributes, slot->key.data,
														 slot->key.bytes, alg);
									...
									size_t* iv_length)
{
	...
	status = psa_driver_wrapper_cipher_set_iv(operation, local_iv, default_iv_length);
								...
								size_t* output_length)
{
	...
	status = psa_driver_wrapper_cipher_encrypt(
		&attributes, slot->key.data, slot->key.bytes, alg, local_iv, default_iv_length, input,
		input_length, output + default_iv_length, output_size - default_iv_length, output_length);
								...
								size_t* output_length)
{
	...
	status =
		psa_driver_wrapper_cipher_decrypt(&attributes, slot->key.data, slot->key.bytes, alg, input,
							  ...
							  size_t* ciphertext_length)
{
	...
	*ciphertext_length = 0;
	...
	status = psa_driver_wrapper_aead_encrypt(&attributes, slot->key.data, slot->key.bytes, alg,
							  ...
							  size_t* plaintext_length)
{
	...
	*plaintext_length = 0;
	...
	status = psa_driver_wrapper_aead_decrypt(&attributes, slot->key.data, slot->key.bytes, alg,
											 nonce, nonce_length, additional_data,
...
static psa_status_t psa_validate_tag_length(psa_aead_operation_t* operation, psa_algorithm_t alg)
{
	uint8_t tag_len = 0;
								   ...
								   psa_key_id_t key, psa_algorithm_t alg)
{
	...
	if(is_encrypt)
		status = psa_driver_wrapper_aead_encrypt_setup(operation, &attributes, slot->key.data,
													   slot->key.bytes, alg);
	else
		status = psa_driver_wrapper_aead_decrypt_setup(operation, &attributes, slot->key.data,
													   slot->key.bytes, alg);
	if(status != PSA_SUCCESS)
		goto exit;
...
/****************************************************************/
	...
	#if defined(BUILTIN_ALG_ANY_HKDF) || defined(IOTEX_PSA_BUILTIN_ALG_TLS12_PRF) ||               \
...
static psa_status_t psa_key_derivation_start_hmac(psa_mac_operation_t* operation,
												  psa_algorithm_t hash_alg, const uint8_t* hmac_key,
...
psa_status_t psa_key_derivation_abort(psa_key_derivation_operation_t* operation)
{
	...
	#if defined(IOTEX_PSA_BUILTIN_ALG_TLS12_PRF) || defined(IOTEX_PSA_BUILTIN_ALG_TLS12_PSK_TO_MS)
...
static psa_status_t psa_key_derivation_hkdf_read(psa_hkdf_key_derivation_t* hkdf,
												 psa_algorithm_t kdf_alg, uint8_t* output,
												 size_t output_length)
{
	...
	#if defined(IOTEX_PSA_BUILTIN_ALG_TLS12_PRF) || defined(IOTEX_PSA_BUILTIN_ALG_TLS12_PSK_TO_MS)
static psa_status_t
	psa_key_derivation_tls12_prf_generate_next_block(psa_tls12_prf_key_derivation_t* tls12_prf,
													 psa_algorithm_t alg)
{
...
static psa_status_t psa_key_derivation_tls12_prf_read(psa_tls12_prf_key_derivation_t* tls12_prf,
													  psa_algorithm_t alg, uint8_t* output,
													  size_t output_length)
{
	...
	psa_key_slot_t* slot, size_t bits, psa_key_derivation_operation_t* operation, uint8_t** data)
{
	...
	iotex_ecp_group_id grp_id = iotex_ecc_group_of_psa(curve, bits, 0);
	...
	size_t bits, psa_key_derivation_operation_t* operation, uint8_t** data)
{
	...
	*data = (uint8_t*)iotex_calloc(1, output_length);
													  ...
													  psa_key_derivation_operation_t* operation)
{
	...
	if(PSA_KEY_TYPE_IS_ECC(slot->attr.type))
	{
		...
		if(key_type_is_raw_bytes(slot->attr.type))
		{
			if(bits % 8 != 0)
				return (PSA_ERROR_INVALID_ARGUMENT);
			data = (uint8_t*)iotex_calloc(1, bytes);
			if(data == NULL)
				return (PSA_ERROR_INSUFFICIENT_MEMORY);
	...
	if(status != PSA_SUCCESS)
		goto exit;
	status = psa_driver_wrapper_import_key(&attributes, data, bytes, slot->key.data,
										   slot->key.bytes, &slot->key.bytes, &bits);
										   ...
										   psa_key_id_t* key)
{
	...
	*key = IOTEX_SVC_KEY_ID_INIT;
	...
	if(status == PSA_SUCCESS)
	{
		status = psa_generate_derived_key_internal(slot, attributes->core.bits, operation);
	}
								   ...
								   size_t data_length)
{
	...
	switch(step)
	{
		...
		case PSA_KEY_DERIVATION_INPUT_SECRET:
		...
		#endif /* IOTEX_PSA_BUILTIN_ALG_HKDF_EXPAND */
			{
				...
				if(status != PSA_SUCCESS)
					return (status);
				status =
					psa_mac_sign_finish(&hkdf->hmac, hkdf->prk, sizeof(hkdf->prk), &data_length);
				if(status != PSA_SUCCESS)
					return (status);
			}
		...
		case PSA_KEY_DERIVATION_INPUT_INFO:
			...
			if(data_length != 0)
			{
				hkdf->info = (uint8_t*)iotex_calloc(1, data_length);
				if(hkdf->info == NULL)
					return (PSA_ERROR_INSUFFICIENT_MEMORY);
										   ...
										   size_t data_length)
{
	...
	if(data_length != 0)
	{
		prf->seed = (uint8_t*)iotex_calloc(1, data_length);
		if(prf->seed == NULL)
			return (PSA_ERROR_INSUFFICIENT_MEMORY);
										  ...
										  size_t data_length)
{
	...
	if(data_length != 0)
	{
		prf->secret = (uint8_t*)iotex_calloc(1, data_length);
		if(prf->secret == NULL)
			return (PSA_ERROR_INSUFFICIENT_MEMORY);
											...
											const uint8_t* data, size_t data_length)
{
	...
	if(data_length != 0)
	{
		prf->label = (uint8_t*)iotex_calloc(1, data_length);
		if(prf->label == NULL)
			return (PSA_ERROR_INSUFFICIENT_MEMORY);
													...
													const uint8_t* data, size_t data_length)
{
	...
	uint8_t* pms = (uint8_t*)iotex_calloc(1, pms_len);
	if(pms == NULL)
		return (PSA_ERROR_INSUFFICIENT_MEMORY);
	uint8_t* cur = pms;
														  ...
														  const uint8_t* data, size_t data_length)
{
	...
	if(data_length != 0)
	{
		prf->other_secret = (uint8_t*)iotex_calloc(1, data_length);
...
static psa_status_t psa_key_derivation_input_internal(psa_key_derivation_operation_t* operation,
													  psa_key_derivation_step_t step,
													  psa_key_type_t key_type, const uint8_t* data,
													  size_t data_length)
{
	psa_status_t status;
	psa_algorithm_t kdf_alg = psa_key_derivation_get_kdf_alg(operation);
...
{
	return (psa_key_derivation_input_internal(operation, step, PSA_KEY_TYPE_NONE, (uint8_t*)&value,
											  sizeof(value)));
}
...
static psa_status_t psa_key_agreement_ecdh(const uint8_t* peer_key, size_t peer_key_length,
										   const iotex_ecp_keypair* our_key, uint8_t* shared_secret,
										   size_t shared_secret_size, size_t* shared_secret_length)
{
	...
	status = iotex_psa_ecp_load_representation(PSA_KEY_TYPE_ECC_PUBLIC_KEY(curve), bits, peer_key,
											   peer_key_length, &their_key);
	if(status != PSA_SUCCESS)
		goto exit;

	status = iotex_to_psa_error(iotex_ecdh_get_params(&ecdh, their_key, IOTEX_ECDH_THEIRS));
	if(status != PSA_SUCCESS)
		goto exit;
	status = iotex_to_psa_error(iotex_ecdh_get_params(&ecdh, our_key, IOTEX_ECDH_OURS));
	if(status != PSA_SUCCESS)
		goto exit;

	status = iotex_to_psa_error(iotex_ecdh_calc_secret(&ecdh, shared_secret_length, shared_secret,
													   shared_secret_size, iotex_psa_get_random,
	...
	switch(alg)
	{
		...
		case PSA_ALG_ECDH:
			...
			psa_status_t status = iotex_psa_ecp_load_representation(
				private_key->attr.type, private_key->attr.bits, private_key->key.data,
				private_key->key.bytes, &ecp);
			if(status != PSA_SUCCESS)
				return (status);
			status = psa_key_agreement_ecdh(peer_key, peer_key_length, ecp, shared_secret,
...
static psa_status_t psa_key_agreement_internal(psa_key_derivation_operation_t* operation,
											   psa_key_derivation_step_t step,
											   psa_key_slot_t* private_key, const uint8_t* peer_key,
											   size_t peer_key_length)
{
	 ...
	 * secret. */
	status =
		psa_key_agreement_raw_internal(ka_alg, private_key, peer_key, peer_key_length,
...
psa_status_t psa_key_derivation_key_agreement(psa_key_derivation_operation_t* operation,
											  psa_key_derivation_step_t step,
											  psa_key_id_t private_key, const uint8_t* peer_key,
											  size_t peer_key_length)
{
	psa_status_t status = PSA_ERROR_CORRUPTION_DETECTED;
	psa_status_t unlock_status = PSA_ERROR_CORRUPTION_DETECTED;
								   ...
								   size_t output_size, size_t* output_length)
{
	...
	size_t expected_length = PSA_RAW_KEY_AGREEMENT_OUTPUT_SIZE(slot->attr.type, slot->attr.bits);
...
static void iotex_psa_random_init(iotex_psa_random_context_t* rng)
{
	...
	rng->entropy_init(&rng->entropy);
		#if defined(IOTEX_PSA_INJECT_ENTROPY) && defined(IOTEX_NO_DEFAULT_ENTROPY_SOURCES)
	/* The PSA entropy injection feature depends on using NV seed as an entropy
	 * source. Add NV seed as an entropy source for PSA entropy injection. */
	iotex_entropy_add_source(&rng->entropy, iotex_nv_seed_poll, NULL, IOTEX_ENTROPY_BLOCK_SIZE,
...
static psa_status_t iotex_psa_random_seed(iotex_psa_random_context_t* rng)
{
	...
	int ret = iotex_psa_drbg_seed(&rng->entropy, drbg_seed, sizeof(drbg_seed) - 1);
	return iotex_to_psa_error(ret);
	#endif /* IOTEX_PSA_CRYPTO_EXTERNAL_RNG */
}
...
psa_status_t psa_generate_random(uint8_t* output, size_t output_size)
{
	...
	psa_status_t status =
		iotex_psa_external_get_random(&global_data.rng, output, output_size, &output_length);
	if(status != PSA_SUCCESS)
		return (status);
	...
	{
		size_t request_size =
			(output_size > IOTEX_PSA_RANDOM_MAX_REQUEST ? IOTEX_PSA_RANDOM_MAX_REQUEST :
															output_size);
		int ret = iotex_psa_get_random(IOTEX_PSA_RANDOM_STATE, output, request_size);
		if(ret != 0)
			return (iotex_to_psa_error(ret));
...
int iotex_psa_get_random(void* p_rng, unsigned char* output, size_t output_size)
{
	...
	if(status == PSA_SUCCESS)
		...
		return (IOTEX_ERR_ENTROPY_SOURCE_FAILED);
}
	#endif /* IOTEX_PSA_CRYPTO_EXTERNAL_RNG */
...
psa_status_t psa_generate_key_internal(const psa_key_attributes_t* attributes, uint8_t* key_buffer,
									   size_t key_buffer_size, size_t* key_buffer_length)
{
	psa_status_t status = PSA_ERROR_CORRUPTION_DETECTED;
	psa_key_type_t type = attributes->core.type;

	if((attributes->domain_parameters == NULL) && (attributes->domain_parameters_size != 0))
		return (PSA_ERROR_INVALID_ARGUMENT);
	...
	#if defined(IOTEX_PSA_BUILTIN_KEY_TYPE_RSA_KEY_PAIR) && defined(IOTEX_GENPRIME)
		if(type == PSA_KEY_TYPE_RSA_KEY_PAIR)
	{
		return (
			iotex_psa_rsa_generate_key(attributes, key_buffer, key_buffer_size, key_buffer_length));
	}
	else
	#endif /* defined(IOTEX_PSA_BUILTIN_KEY_TYPE_RSA_KEY_PAIR)                                     \
	...
	{
		return (
			iotex_psa_ecp_generate_key(attributes, key_buffer, key_buffer_size, key_buffer_length));
	}
	else
	#endif /* defined(IOTEX_PSA_BUILTIN_KEY_TYPE_ECC_KEY_PAIR) */
	{
...
psa_status_t psa_generate_key(const psa_key_attributes_t* attributes, psa_key_id_t* key)
{
	...
	*key = IOTEX_SVC_KEY_ID_INIT;
	...
	if(slot->key.data == NULL)
	{
		...
		{
			status = psa_validate_key_type_and_size_for_key_generation(attributes->core.type,
																	   attributes->core.bits);
			if(status != PSA_SUCCESS)
				goto exit;

			key_buffer_size =
				PSA_EXPORT_KEY_OUTPUT_SIZE(attributes->core.type, attributes->core.bits);
		}
		else
		{
	...
	status = psa_driver_wrapper_generate_key(attributes, slot->key.data, slot->key.bytes,
											 &slot->key.bytes);
	if(status != PSA_SUCCESS)
...
int iotex_mpi_add_int(iotex_mpi* X, const iotex_mpi* A, iotex_mpi_sint b)
{
	return -1;
}

int iotex_mpi_lt_mpi_ct(const iotex_mpi* X, const iotex_mpi* Y, unsigned* ret)
{
	return -1;
}

int iotex_mpi_sub_int(iotex_mpi* X, const iotex_mpi* A, iotex_mpi_sint b)
{
	return -1;
}
